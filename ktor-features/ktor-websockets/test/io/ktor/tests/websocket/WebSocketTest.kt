    ...
    fun testConversationTwoUsers() {
        withTestApplicationSuspend {
            ...
            application.routing {
                ...
                class Chat {
                    ...
                    suspend fun broadcast(s: String) {
                        for (session in synchronized(this) { sessions.toList() }) {
                            session.outgoing.send(Frame.Text(s))
                        }
                    }
            ...
            handleWebSocketConversation("/chat") { incoming1, outgoing1 ->
                log1 += "[1]" + (incoming1.receive() as Frame.Text).readText()
                handleWebSocketConversation("/chat") { incoming2, outgoing2 ->
                    outgoing1.send(Frame.Text("HELLO"))
                    outgoing2.send(Frame.Text("HI"))
                    for (n in 0 until 3) log1 += "[1]" + (incoming1.receive() as Frame.Text).readText()
                    for (n in 0 until 3) log2 += "[2]" + (incoming2.receive() as Frame.Text).readText()
                }
            }
